...
```typescript
// ❌ ZOMBIE: Feature flag never enabled
...
```tsx
// ❌ ZOMBIE: Entire component commented out
{/* 
<BetaFeature>
  // 200 lines of JSX
</BetaFeature>
...
```typescript
// 🌟 HIDDEN GEM: Found in src/features/user/helpers.ts
...
// ✅ RECOMMENDATION: Promote to shared library
...
```typescript
// 🌟 DUPLICATED GEM: Same validation logic in 4 places
...
```yaml
# 🚨 SILENT BLOCKER: Undocumented dependency
...
```yaml
# 🚨 SILENT BLOCKER: Hidden global dependency
...
async function fetchWithRetry(url: string) {
  for (let i = 0; i < 5; i++) {  // ❌ Why 5?
    try {
      return await fetch(url);
    } catch (error) {
      await sleep(3000);  // ❌ Why 3 seconds?
    }
  }
...
// ✅ FIX:
const MAX_RETRIES = 5;  // ✅ Reasonable default for transient network errors
const RETRY_DELAY_MS = 3000;  // ✅ AWS Lambda cold start typically < 3s
...
```csharp
// 🚨 HARDCODED PATH: Fails outside Windows
// Found in: FileService.cs

public void SaveUpload(IFormFile file) {
    var path = @"C:\inetpub\uploads\" + file.FileName;  // ❌ Windows-only
...
```yaml
# 🚨 MANUAL WORKFLOW: Deployment nightmare
...
# ROI: Save 45 min × 2 deploys/week × 52 weeks = 78 hours/year
```
...
```yaml
# 🚨 BUS FACTOR: Critical module, single maintainer
...
# ❌ Breaks if run from subdirectory
cd src/
npm run build

# ❌ No error handling - continues on failure
docker build -t myapp .
docker push myapp:latest

# ❌ Assumes docker login already done
# ❌ No rollback on failure
# ❌ Not idempotent (re-run breaks)
...
## Discovery Summary
- 🧹 Zombie code: 1,200 lines (3 files)
- 💎 Hidden gems: 4 functions to promote
- 🚨 Silent blockers: 2 undocumented dependencies
- 🔧 Manual workflows: 1 prime for automation
- 🚌 Bus factor: 3 critical single-maintainer modules

---

## Zombie Code (1,200 lines to delete)

### 1. Premium Dashboard (400 lines) 🧹
...
### 1. Undocumented libvips Dependency 🚨
...
### 1. Manual Deployment (30 steps → 1 click) 🔧
**Current**: 45-minute manual process
**Found In**: `DEPLOYMENT.md`

**ROI Calculation**:
- Current: 45 min × 2 deployments/week = 90 min/week
- Annual: 90 min × 52 weeks = 78 hours
...
### 1. Payment Service (Bus Factor: 1) 🚌
