...
�?zelliklerden otomatik test senaryoları olu�?turur, bu senaryolardan çalı�?tırılabilir test kodları üretir ve testleri execute edip kapsamlı raporlar sunar. Feature Gap analizi sonrası eklenen yeni özellikler veya mevcut özelliklerin do�?ru çalı�?ıp çalı�?madı�?ını BDD-style senaryolarla do�?rular.
...
common_issues:
  missing_tests:
    - "Yeni özellikler ekledim ama test yazmadım"
    - "Hangi özelliklerin test edilmedi�?ini bilmiyorum"
    - "Test coverage %30'un altında"
  
  inadequate_tests:
    - "Testler var ama yüzeysel"
    - "Sadece happy path test edilmi�?"
    - "Edge case'ler eksik"
    - "Integration testleri yok"
  
  unclear_requirements:
    - "�?zellik beklendi�?i gibi çalı�?mıyor"
    - "Do�?ru davranı�?ı bilmiyorum"
    - "Acceptance criteria belirsiz"
  
  manual_testing_burden:
    - "Her de�?i�?iklikte manuel test yapıyorum"
    - "Regression testi uzun sürüyor"
    - "Test senaryoları kafamda"
  
  actor_complexity:
    - "Farklı kullanıcı rolleri için test gerekiyor"
    - "Multi-actor senaryolar karma�?ık"
...
process:
  step_1_feature_analysis:
    action: "�?zellikleri ve kullanıcı hikayelerini analiz et"
    extract:
      - Feature description
      - User roles involved
      - Business rules
  ...
  step_2_actor_identification:
    action: "Actor'ları ve yetkilerini belirle"
    actors:
      admin:
...
process:
  ...
  step_2_code_generation:
    action: "Gherkin senaryolardan test kodu üret"
...
| Metric | Value | Status |
|--------|-------|--------|
| **Tests Passed** | 118 / 127 | �?? 92.9% |
| **Tests Failed** | 9 / 127 | �?�️ 7.1% |
| **Skipped** | 0 | - |
| **Line Coverage** | 78.5% | �??� Good |
| **Branch Coverage** | 65.2% | �??� Good |
| **Function Coverage** | 82.1% | �?? Excellent |
...
- �?? `src/services/authService.js` - 95.2%
- �?? `src/services/productService.js` - 88.7%
- �?? `src/components/LoginForm.tsx` - 91.3%

### Needs Attention (< 60%)
- �?�️ `src/services/paymentService.js` - 45.8%
- �?�️ `src/services/emailService.js` - 38.2%
- �? `src/utils/encryption.js` - 12.1%
...
2. Add tests for password reset (0% �?? 80%)
3. Improve payment service tests (45% �?? 80%)

### Priority 2 (Important)
1. Add edge case tests for cart
2. Increase encryption utility tests (12% �?? 80%)
...
```markdown
"TEST-GEN koduyla yeni eklenen özelliklerin test senaryolarını olu�?tur.
Mode 1: Test senaryoları yaz (Markdown/Gherkin).

�?zellikler:
- Kullanıcı kayıt sistemi (email verification ile)
- �?rün sepete ekleme
- �?deme i�?lemi (Stripe)
...
Türkçe açıklamalarla, Gherkin formatında."
```
...
Adımlar:
1. Mevcut özellikleri analiz et
2. Test senaryoları olu�?tur (Gherkin)
3. Jest testleri generate et (frontend)
4. xUnit testleri generate et (backend)
5. Testleri çalı�?tır
6. Coverage raporu olu�?tur

Eksik testleri tespit et, öner.
Türkçe rapor."
```
...
```markdown
"TEST-GEN koduyla failing testleri analiz et ve düzelt.

Mevcut durum:
- 9 test fail ediyor
- Shopping cart total hesabı yanlı�?
...
2. Root cause bul
3. Fix önerileri sun
4. Test kodlarını güncelle
5. Tekrar çalı�?tır ve validate et"
```
