# Project Type Detection - Otomatik Proje Tipi AlgÄ±lama

**Version**: 1.0  
**Purpose**: YÃ¼klenen dosyalardan proje tipini otomatik algÄ±lama  
**Accuracy**: ~95% (common project types iÃ§in)

---

## ðŸŽ¯ AmaÃ§

KullanÄ±cÄ± "Emin deÄŸilim" dediÄŸinde veya proje tipini belirtmediÄŸinde, sistem otomatik olarak proje tipini algÄ±lar ve doÄŸru modÃ¼lleri yÃ¼kler.

---

## ðŸ” AlgÄ±lama MekanizmasÄ±

### 3-KatmanlÄ± AlgÄ±lama

```yaml
Layer 1: File Signatures (95% accuracy)
  â†’ Belirli dosya varlÄ±ÄŸÄ±na gÃ¶re

Layer 2: Content Analysis (98% accuracy)
  â†’ Dosya iÃ§eriklerine gÃ¶re  

Layer 3: Pattern Matching (99% accuracy)
  â†’ KlasÃ¶r yapÄ±sÄ± + dosya kombinasyonlarÄ±
```
...
// package.json iÃ§inde:
{
  "dependencies": {
    "react": "^18.x",        // âœ… React
    "react-dom": "^18.x"     // âœ… React DOM
  }
}

// App.tsx iÃ§inde:
import React from 'react'  // âœ… React import
export default function App() { ... }  // âœ… Component
```
...
<!-- *.csproj iÃ§inde -->
<PropertyGroup>
  <TargetFramework>net6.0</TargetFramework>  <!-- âœ… .NET 6 -->
</PropertyGroup>

<ItemGroup>
  <PackageReference Include="Microsoft.AspNetCore.App" />  <!-- âœ… ASP.NET -->
</ItemGroup>
```

```csharp
// Program.cs iÃ§inde
var builder = WebApplication.CreateBuilder(args);  // âœ… ASP.NET Core
builder.Services.AddControllers();  // âœ… API
```
...
{
  "dependencies": {
    "express": "^4.x"  // âœ… Express
  },
  "main": "index.js"  // âœ… Entry point
}
```

```javascript
// index.js iÃ§inde
const express = require('express')  // âœ… Express import
const app = express()  // âœ… Express app
app.listen(3000)  // âœ… Server start
```
...
{
  "dependencies": {
    "react-native": "^0.7x"  // âœ… RN
  }
}

// app.json
{
  "expo": { ... }  // âœ… Expo project
}
```

```typescript
// App.tsx
import { View, Text } from 'react-native'  // âœ… RN components
```
...
very_high (90-100):
  ...
  action: "Otomatik modÃ¼l yÃ¼kle"
  example: ".csproj + Controllers/ â†’ .NET API"
  
high (70-89):
  description: "YÃ¼ksek ihtimal doÄŸru"
  action: "ModÃ¼ller yÃ¼kle, kullanÄ±cÄ±ya bildir"
  example: "package.json + express â†’ Express API"
  
medium (50-69):
  description: "Muhtemelen doÄŸru"
  action: "KullanÄ±cÄ±ya sor, onay al"
  example: "package.json alone â†’ Could be anything"
  
low (<50):
  description: "Belirsiz"
  action: "Manuel seÃ§im iste"
  example: "Sadece .js files, no package.json"
```

---

## ðŸŽ¯ Detection Results Format
...
```
5 files â†’ 70% accuracy
15 files â†’ 90% accuracy
25+ files â†’ 95%+ accuracy
...
## ðŸ†˜ When Detection Fails

**User sees**:
```markdown
âš ï¸ Project type unclear (confidence: 45%)
...
**Smart detection saves time! Let the system do the work.** ðŸ¤–
